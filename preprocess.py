...
class Preprocess:
    ...
    def create_new_features(self, df):
        ...
        Location_density = new_df.groupby(['Longitude', 'Latitude']).size().reset_index(name='Location_Density')
    ...
    def count_daily_crime(self, leaf_data_frames):
        for leaf_df  in leaf_data_frames:
            # Combine CMPLNT_FR_DT, Longitude, and Latitude to create a unique identifier for each crime event
            # leaf_df['Unique_Crime'] = leaf_df['CMPLNT_FR_DT'].astype(str) + '_' + leaf_df['Longitude'].astype(str) + '_' + leaf_df['Latitude'].astype(str)
            leaf_df['Unique_Crime'] = leaf_df['CMPLNT_FR_DT'].astype(str)

            # Count the number of occurrences of each unique crime event per day
            leaf_df['Daily_Crime_Count'] = leaf_df.groupby('Unique_Crime')['Unique_Crime'].transform('count')
            
            # Drop the 'Unique_Crime' column as it's no longer needed
            leaf_df.drop(columns=['Unique_Crime'], inplace=True)
